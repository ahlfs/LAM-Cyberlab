 ...
 */
export const baselineSnapshots = new Map<
  string,
 ...
 */
async function getGitScmStatus(
  dirPath: string,
): Promise<{
  isGit: boolean
  ...
  try {
    ...
    let gitRoot = ''
    try {
      const { stdout: gitRootRaw } = await execFileAsync(
        'git',
 ...
 */
async function getAiModifiedFiles(
  dirPath: string,
        ...
        try {
          ...
          if (action === 'changed-files') {
            const targetDir = inputPath
              ? ensureWorkspacePath(inputPath, workspaceRoot)
              : workspaceRoot
          ...
          return json(
            { ok: false, error: error?.message || 'Failed to get file diff' },
            { status: 500 },
          )
        }
      },
        ...
        try {
          ...
          if (body.action === 'accept-all') {
            ...
            for (const f of files) {
              const fPath = ensureWorkspacePath(
                f.path || (f as any),
                workspaceRoot,
              )
              try {
          ...
          if (
            body.action === 'accept' &&
            typeof body.modifiedContent === 'string'
          ) {
          ...
          return json(
            { ok: false, error: 'Invalid action or missing content' },
            { status: 400 },
          )
        } catch (error: any) {
          return json(
            { ok: false, error: error?.message || 'Failed to process action' },
            { status: 500 },
          )
        }
      },
