...
this machine (it isn't, by default, on this Debian box — install with
`sudo apt install tesseract-ocr` to enable). Without it, image files are
skipped with a clear message rather than silently producing nothing.

Run via the isolated venv:
    .venv-second-brain/bin/python scripts/ingest_docs.py

Env vars:
    SECOND_BRAIN_VAULT  path to the Obsidian vault (default: ~/obsidian/memo)
"""
import datetime
import os
import shutil
