...
def mock_resolve_runtime(*args, **kwargs):
    ...
    print(f"DEBUG _resolve_runtime_agent_kwargs returned provider={res.get('provider')}")
    return res
gateway.run._resolve_runtime_agent_kwargs = mock_resolve_runtime

orig_resolve_provider_runtime = None
def mock_apply_overrides(runtime_kwargs, overrides):
    print(f"DEBUG _apply_runtime_agent_overrides called with overrides={overrides}")
    return _apply_runtime_agent_overrides(runtime_kwargs, overrides)

import gateway.platforms.api_server
gateway.platforms.api_server._apply_runtime_agent_overrides = mock_apply_overrides
